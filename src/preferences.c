...
void preferences_set (Preferences* self, const gchar* name, GVariant* value) {
	...
	if (_tmp3_) {
		...
		if (_tmp11_) {
			...
			_tmp20_ = name;
			_tmp21_ = value;
			ibus_config_set_value (_tmp19_, "engine/kkc", _tmp20_, _tmp21_);
		} else {
			GeeMap* _tmp22_ = NULL;
			const gchar* _tmp23_ = NULL;
			IBusConfig* _tmp24_ = NULL;
			const gchar* _tmp25_ = NULL;
			_tmp22_ = self->priv->current;
			_tmp23_ = name;
			gee_map_unset (_tmp22_, _tmp23_, NULL);
			_tmp24_ = self->priv->config;
			_tmp25_ = name;
			ibus_config_unset (_tmp24_, "engine/kkc", _tmp25_);
		}
	}
	_g_variant_unref0 (_value);
}
...
static void _preferences_value_changed_cb_ibus_config_value_changed (IBusConfig* _sender, const gchar* p0, const gchar* p1, GVariant* p2, gpointer self) {
	preferences_value_changed_cb ((Preferences*) self, _sender, p0, p1, p2);
}
...
static void preferences_value_changed_cb (Preferences* self, IBusConfig* config, const gchar* section, const gchar* name, GVariant* value) {
	...
	if (g_strcmp0 (_tmp0_, "engine/kkc") == 0) {
		...
		_tmp14_ = name;
		_tmp15_ = value;
		g_signal_emit_by_name (self, "value-changed", _tmp14_, _tmp15_);
	}
}
...
static void preferences_instance_init (Preferences * self) {
	...
	_tmp0_ = gee_hash_map_new (G_TYPE_STRING, (GBoxedCopyFunc) g_strdup, g_free, G_TYPE_VARIANT, (GBoxedCopyFunc) g_variant_ref, g_variant_unref, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL);
	self->priv->_default = (GeeMap*) _tmp0_;
	_tmp1_ = gee_hash_map_new (G_TYPE_STRING, (GBoxedCopyFunc) g_strdup, g_free, G_TYPE_VARIANT, (GBoxedCopyFunc) g_variant_ref, g_variant_unref, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL);
